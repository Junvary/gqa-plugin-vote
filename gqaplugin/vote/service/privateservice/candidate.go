...
func CandidateList(getCandidateList model.RequestCandidateList, username string) (err error, candidate []model.GqaPluginVoteCandidate, total int64) {
	...
	var db *gorm.DB
	db = gqaGlobal.GqaDb.Model(&model.GqaPluginVoteCandidate{})
	//配置搜索
	...
	err = db.Limit(pageSize).Offset(offset).Order(gqaModel.OrderByColumn("candidate", false)).Preload("CandidateByUser").Find(&candidateList).Error
...
func CandidateAdd(toAddCandidate *model.RequestAddCandidate, username string) (err error) {
	...
	for _, v := range toAddCandidate.Candidate {
		ac := model.GqaPluginVoteCandidate{
			...
			CandidateByUser: gqaModel.SysUser{Username: v},
			CreatedBy:       username,
		}
		addCandidate = append(addCandidate, ac)
	}
	if len(addCandidate) != 0 {
		err = gqaGlobal.GqaDb.Model(&model.GqaPluginVoteCandidate{}).Save(&addCandidate).Error
		return err
	} else {
