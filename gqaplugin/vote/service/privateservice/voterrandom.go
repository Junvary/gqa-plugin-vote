...
func VoterRandomList(getVoterRandomList model.RequestVoterRandomList, username string) (err error, voter []model.GqaPluginVoteVoterRandom, total int64) {
	...
	var db *gorm.DB
	db = gqaGlobal.GqaDb.Model(&model.GqaPluginVoteVoterRandom{})
	//配置搜索
	...
	err = db.Limit(pageSize).Offset(offset).
		Order(gqaModel.OrderByColumn("created_at", true)).Order(gqaModel.OrderByColumn("voter", false)).
		Preload("VoterByUser").Find(&voterRandomList).Error
	return err, voterRandomList, total
}

func VoterRandomGet(getVoterRandomNumber model.RequestGetVoterRandom, username string) (err error, voter []gqaModel.SysUser, total int64) {
	//选出这个投票类型的固定投票人
	var baseVoterList []model.GqaPluginVoteVoter
	if err = gqaGlobal.GqaDb.Where("vote_type = ?", getVoterRandomNumber.VoteType).Find(&baseVoterList).Error; err != nil {
		return err, nil, 0
	}
	//计算固定投票人数量
	...
	if getVoterRandomNumber.RandomNumber == 0 {
		...
		//找出所有用户
		var allUserList []gqaModel.SysUser
		if err = gqaGlobal.GqaDb.Where("username != ?", "admin").Find(&allUserList).Error; err != nil {
			return err, nil, 0
		}
		//计算所有用户数量
		...
		if len(baseVoterUsernameList) != 0 {
			//取出剩余用户
			db = gqaGlobal.GqaDb.Where("username != ? and username not in ?", "admin", baseVoterUsernameList).Find(&gqaModel.SysUser{})
		} else {
			//取出剩余用户，没有固定用户时，baseVoterUsernameList的值为null，忽略sql的not in 语句。
			db = gqaGlobal.GqaDb.Where("username != ?", "admin").Find(&gqaModel.SysUser{})
		}
		//按数量随机选取用户
		var surplusUser []gqaModel.SysUser
...
func VoterRandomAdd(toAddVoterRandom *model.RequestAddVoterRandom, username string) (err error) {
	...
	if len(addVoterRandom) != 0 {
		err = gqaGlobal.GqaDb.Model(&model.GqaPluginVoteVoterRandom{}).Save(&addVoterRandom).Error
		return err
	} else {
