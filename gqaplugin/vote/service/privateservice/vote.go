...
func VoteHandle(toAddScore *model.RequestAddScore, username string) (err error) {
	...
	if err = gqaGlobal.GqaDb.Where("vote_type = ? and vote_month = ? and vote_from = ?", voteType, voteMonth, username).
	...
	var dictRatio gqaModel.SysDict
	result := gqaGlobal.GqaDb.Where("dict_code = ?", ratio).First(&dictRatio)
	var voteRatio string
	var ratio8 uint8
	//如果没有找到记录，说明是随机选取的人员，那么按照投票分类的指定投票权重分配
	if errors.Is(result.Error, gorm.ErrRecordNotFound) {
		var ramdomVoterResult gqaModel.SysDict
		if voteType == "dy" {
			if err = gqaGlobal.GqaDb.Where("dict_ext2 = ?", "dy_base").First(&ramdomVoterResult).Error; err != nil {
				return err
			}
		} else {
			if err = gqaGlobal.GqaDb.Where("dict_ext2 = ?", "gl_base").First(&ramdomVoterResult).Error; err != nil {
				return err
			}
		}
	...
	for _, v := range toAddScore.RequestAddScoreDetail {
		as := model.GqaPluginVoteScoreResult{
			...
			CandidateByUser: gqaModel.SysUser{Username: v.Candidate},
			VoteType:        v.VoteType,
	...
	if len(addScore) != 0 {
		err = gqaGlobal.GqaDb.Model(&model.GqaPluginVoteScoreResult{}).Save(&addScore).Error
		return err
	} else {
...
func CanVoteDy(username string) (err error, canVoteResultList interface{}) {
	...
	var alreadyVote int64
	if err = gqaGlobal.GqaDb.Where("vote_type = ? and vote_month = ? and vote_from = ?", "dy", voteMonth, username).
		Find(&model.GqaPluginVoteScoreResult{}).Count(&alreadyVote).Error; err != nil {
		return err, nil
	}
	...
	if err = gqaGlobal.GqaDb.Where("vote_type = ? and vote_ratio = ?", "dy", "dy_jijian").Find(&jiJianList).Error; err != nil {
		return err, nil
	}
	...
	if err = gqaGlobal.GqaDb.Where("vote_type = ? and vote_ratio = ?", "dy", "dy_zhenggong").Find(&zhengGongList).Error; err != nil {
		return err, nil
	}
...
func CanVoteGl(username string) (err error, canVoteResultList interface{}) {
	...
	var alreadyVote int64
	var userInVoter bool
	if err = gqaGlobal.GqaDb.Where("vote_type = ? and vote_month = ? and vote_from = ?", "gl", voteMonth, username).
		Find(&model.GqaPluginVoteScoreResult{}).Count(&alreadyVote).Error; err != nil {
		return err, nil
	}
...
func checkUserInVoterRandom(username string, voteType string) (ifIn bool) {
	...
	gqaGlobal.GqaDb.Where("vote_type = ?", voteType).Order(gqaModel.OrderByColumn("created_at", true)).First(&userInVoter)
	voteVersion := userInVoter.VoteVersion
	gqaGlobal.GqaDb.Where("vote_version = ?", voteVersion).Find(&userInVoterList)
